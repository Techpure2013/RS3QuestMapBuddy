...
app.post("/api/questName", (req, res) => {
  try {
    ...
    console.log(
      `Received quest data for userID: ${userID}, questName: ${questName}`
    );
    res.status(200).json({
...
io.on("connection", (socket) => {
  ...
  socket.on("generateURL", (data) => {
    ...
    const url = `alt1://browser/http://localhost:3000/${userID
      .replace('"', "")
      .replace('"', "")}/${questName.replace(
      " ",
      "_"
    )}/${level}-${z}-${x}-${y}`;
    console.log(
      `Generated URL for userID: ${userID} -> ${url
        .replace('"', "")
        .replace('"', "")}`
    );
  ...
  socket.on("removeTempURL", (data) => {
    ...
    userSessions.set(userID, session);
    console.log(
      `Temporary URL for userID: ${userID} and questName: ${questName} has been removed.`
    );
