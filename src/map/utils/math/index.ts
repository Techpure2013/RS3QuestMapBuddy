...
export function circularMean(angles: number[]): number {
  return Math.atan2(
    lodash.sum(angles.map(Math.sin)),
    lodash.sum(angles.map(Math.cos))
  );
}
...
export function rectangleCrossSection(
  size: Vector2,
  ...
  const sizes = diagonal_angles.map((diag_angle) => {
    ...
    if (attack_angle_on_diagonal > Math.PI)
      attack_angle_on_diagonal = 2 * Math.PI - attack_angle_on_diagonal;
    if (attack_angle_on_diagonal > Math.PI / 2)
      attack_angle_on_diagonal = Math.PI - attack_angle_on_diagonal;
