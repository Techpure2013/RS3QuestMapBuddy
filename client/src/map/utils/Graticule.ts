...
export default class Graticule extends leaflet.FeatureGroup {
  ...
  redraw() {
    ...
    let interval = lodash.minBy(
      this._options.intervals.filter((i) => this._map.getZoom() >= i.min_zoom),
      (i) => i.interval
    );

    if (
      !this.last_drawn ||
      !this.last_drawn.bounds.contains(bounds) ||
      this.last_drawn.interval != interval?.interval
    ) {
  ...
  constructLines(bounds: leaflet.LatLngBounds, interval: number) {
    ...
    new leaflet.Polyline(lines, this._style).addTo(this);
  }
}
